...
const inventoryData = Papa.parse(inventoryCSV, { header: true, skipEmptyLines: true });
    ...
    if (stockMatch) {
      ...
      for (let i = index - 2; i <= index + 2; i++) {
        if (i >= 0 && i < excelHeaders.length && excelHeaders[i]) {
          ...
          if ((nearbyHeader.toLowerCase().includes('restock') ||
...
for (let i = 3; i < jsonData.length; i++) {
  ...
  dateColumns.forEach((dateCol, idx) => {
    ...
    const restockAmount = dateCol.restockIndex >= 0 ? row[dateCol.restockIndex] : 0;
    ...
    const consumption = previousStock + restock - currentStock;
    ...
    const timestamp = new Date(year, parseInt(month) - 1, parseInt(day), 12, 0, 0).toISOString();
    ...
    const transaction = {
      ...
      status: 'completed',
      notes: `Imported from Excel - ${dateCol.date}${dateCol.restockHeader ? ` (${dateCol.restockHeader})` : ''}`
    };
...
try {
  ...
  existingTransactions = parsed.data.filter(t => t.id && !t.notes?.includes('Imported from Excel'));
  console.log(`Found ${existingTransactions.length} existing transactions to preserve`);
} catch (err) {
...
console.log(`✅ Successfully imported ${transactions.length} historical transactions to ${transactionsPath}`);

// Show sample
console.log(`\nSample transactions:`);
transactions.slice(0, 5).forEach(t => {
  const item = inventory.find(i => i.id === t.inventory_id);
  console.log(`- ${item?.item || 'Unknown'}: ${t.previous_stock} → ${t.new_stock} (received: ${t.actual_received}, consumed: ${t.consumption}) on ${new Date(t.timestamp).toLocaleDateString()}`);
});
